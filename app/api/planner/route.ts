...
export async function POST(request: Request) {
  try {
    ...
    const prompt = `你是严谨、克制的私人股权学习规划师。阅读用户描述，提取学习规划参数，并审阅系统依据工作日/周末时间容量生成的每日任务。用户选择的书：${body.books.join('、')}。不得改动日期、阅读时间或章节范围；只补充简洁的学习重点与成果。所有 focus 和 outcome 必须只用简体中文，即使原书是英文。每周生成 focus（不超过16个中文字）和 outcome（不超过45个中文字）；每个有阅读任务的日期生成 focus（不超过14个中文字）和 outcome（不超过28个中文字）。每日 outcome 必须依据下方对应章节正文，不得复用其他章节的内容；若正文未提供，只写“阅读本章核心内容”，不要猜测。若用户明确说“不需要复盘或休息”，绝不建议复盘、休息或留空日。成果要说明当天可掌握的知识或判断，不要泛泛谈面试。只返回合法 JSON，不要 markdown：{"weeks":12,"weekdayTime":"...","weekendTime":"...","background":"...","goal":"...","weeklySummaries":[{"week":1,"focus":"...","outcome":"..."}],"dailySummaries":[{"week":1,"day":1,"focus":"...","outcome":"..."}]}。weeks 必须是 2-52 的整数；对用户明确给出的周期和时间必须原样尊重。\n\n用户描述：${body.brief}\n\n每周章节安排：${JSON.stringify(body.draftSchedule || [])}\n\n每日安排：${JSON.stringify(body.draftDailySchedule || [])}\n\n对应章节正文摘录：${JSON.stringify(body.chapterContexts || [])}`;
    const endpoint =
