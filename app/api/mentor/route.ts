...
type MentorRequest = { apiKey:string; defaultModel?:DeepSeekModelTier; autoRoute?:boolean; task?:'mentor'; thinking?:'enabled'|'disabled'; reasoningEffort?:'low'|'medium'|'high'|'max'; book:string; page:number; quote:string; question:string; mode:string };
...
export async function POST(request:Request) {
  try {
    ...
    const { response, data, model } = await callDeepSeek(body, { messages:[{role:'system',content:SYSTEM},{role:'user',content:prompt}], max_tokens:500 });
    if (!response.ok) return Response.json({error:deepSeekErrorMessage(response.status, data.error?.message)},{status:response.status});
    return Response.json({answer:data.choices?.[0]?.message?.content||'导师没有返回内容。',usage:data.usage,model});
  } catch(error) { return Response.json({error:error instanceof Error && error.message === 'MISSING_API_KEY' ? '缺少 API Key、原文或问题。' : '无法处理请求。'},{status:500}); }
}
